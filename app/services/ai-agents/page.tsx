          ...
          {filteredAgents.map((agent) => {
            ...
            return <article key={agent.name} className="group flex min-h-[270px] flex-col rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-lg">
              ...
              <details className="mt-5 border-t border-border pt-4">
                <summary className="flex cursor-pointer list-none items-center justify-between text-sm font-semibold text-primary">View profile <ArrowUpRight aria-hidden="true" /></summary>
                <div className="flex flex-col gap-4 pt-5 text-sm leading-6">
                  ...
                  <Link href={contactLinks.email} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 font-semibold text-primary-foreground transition-colors hover:bg-primary/90">Deploy this agent <ArrowUpRight aria-hidden="true" /></Link>
                </div>
              </details>
            </article>
          })}
